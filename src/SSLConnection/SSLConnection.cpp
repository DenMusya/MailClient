...
std::shared_ptr<SSLConnection> SSLConnection::create(boost::asio::io_context& io) {
  return std::shared_ptr<SSLConnection>(new SSLConnection(io));
}

SSLConnection::SSLConnection(boost::asio::io_context& io)
    : io_(io), ctx_(ssl::context::tlsv12_client), resolver_(io), stream_(io, ctx_) {
...
awaitable_result<void> SSLConnection::asyncConnect(const std::string& host,
  ...
  try {
    auto results = co_await resolver_.async_resolve(host, port, boost::asio::use_awaitable);
    ...
    co_return std::unexpected(err.code());
  }
}
